...
TEST(LedDriver , LedsOffAfterCreate){
    leds = expected(0xffff);
    LedDriver_Create(&leds, levels);
    LONGS_EQUAL(expected(0), leds);
}

TEST(LedDriver, TurnOnLedOne){
    LedDriver_TurnOn(1);
    LONGS_EQUAL(expected(1), leds);
}
...
TEST(LedDriver, TurnOnMultipleLeds){
    LedDriver_TurnOn(9);
    LedDriver_TurnOn(8);
    LONGS_EQUAL(expected(0x180), leds);
}

TEST(LedDriver, AllOn){
    LedDriver_TurnOnAll();
    LONGS_EQUAL(expected(0xffff), leds);
}
...
TEST(LedDriver, TurnOffAnyLed){
    ...
    LedDriver_TurnOn(8);
    LedDriver_TurnOff(8);
    LONGS_EQUAL(expected(0xff7f), leds);
}

TEST(LedDriver, LedMemoryNotReadable){
    leds = expected(0xffff);
    LedDriver_TurnOn(8);
    LONGS_EQUAL(expected(0x0080), leds);

    leds = expected(0xffff);
    LedDriver_TurnOff(8);
    LONGS_EQUAL(expected(0x0000), leds);
}

TEST(LedDriver, UpperAndLowerBounds){
    LedDriver_TurnOn(1);
    LedDriver_TurnOn(16);
    LONGS_EQUAL(expected(0x8001), leds);
}
...
TEST(LedDriver, TurnOffMultipleLeds){
    ...
    LedDriver_TurnOff(9);
    LedDriver_TurnOff(8);
    LONGS_EQUAL(expected(0xfe7f), leds);
}

TEST(LedDriver, AllOff){
    LedDriver_TurnOnAll();
    LedDriver_TurnOffAll();
    LONGS_EQUAL(expected(0x0), leds);
}
